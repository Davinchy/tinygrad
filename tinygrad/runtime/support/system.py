...
import os, mmap, array, functools, ctypes, ctypes.util, select, contextlib, dataclasses, sys, struct, socket, itertools, subprocess, time, enum
import atexit
from tinygrad.device import BufferStorage, Buffer, Device
from tinygrad.helpers import round_up, getenv, OSX, temp, ceildiv, DEBUG, pluralize, unwrap, fetch, system, _ensure_downloads_dir, flatten
...
class PCIDevice:
  ...
  # sysmem given out by alloc_sysmem is the only host memory a remote device can dma: these resolve and recycle it
...
class PCIIfaceBase:
  ...
  def map(self, b:Buffer) -> BufferStorage:
    if b.device.split(":")[0] in {"CPU", "PYTHON", "NPY"}:
      ...
      if not self.is_local(): # only pages the remote's dma allocator handed out are mappable, and the gpu sees them at a va of their own
        paddrs, aspace, snooped, uncached = [(x, 0x1000) for x in self.pci_dev.sysmem_paddrs(lo, size)], AddrSpace.SYS, True, True
        lo = self.dev_impl.mm.alloc_vaddr(size)
      else:
    ...
    return BufferStorage(lo, (lo, size))
...
REMOTE_REQ, REMOTE_RESP = '<BIIQQQ', '<BQQ' # request: cmd, dev_id, bar, arg0, arg1, arg2 (33 bytes). response: status, resp0, resp1 (17 bytes)

class RemoteMMIOInterface(MMIOInterface):
  # there is no cpu mapping: every access is a round trip. addr is the byte offset inside the resource, so views and addr arithmetic still work
...
class RemotePCIDevice(PCIDevice):
  ...
  def remote_sock(host:str, port:int) -> socket.socket:
    ...
    sock.settimeout(RemotePCIDevice.timeout)
    sock.connect((host, port))
    sock.settimeout(None)
    RemotePCIDevice._stats()
    return sock
  ...
  def _recvall(sock:socket.socket, n:int) -> bytes:
    data = b''
    while len(data) < n:
      if not select.select([sock], [], [], RemotePCIDevice.timeout)[0]: raise TimeoutError(f"remote: no response within {RemotePCIDevice.timeout}s")
      if not (chunk:=sock.recv(n - len(data))): raise RuntimeError("Connection closed")
      data += chunk
    return data

  @staticmethod
  def _rpc(sock:socket.socket, dev_id:int, cmd:int, *args:int, bar:int=0, readout_size:int=0, payload:bytes=b'', has_fd=False):
    sock.sendall(struct.pack(REMOTE_REQ, cmd, dev_id, bar, *(*args, 0, 0, 0)[:3]) + payload)
    if has_fd:
      if not select.select([sock], [], [], RemotePCIDevice.timeout)[0]: raise TimeoutError(f"remote: no response within {RemotePCIDevice.timeout}s")
      msg, fds, _, _ = socket.recv_fds(sock, struct.calcsize(REMOTE_RESP), 1)
      fd = fds[0] if fds else None
    else: msg, fd = RemotePCIDevice._recvall(sock, struct.calcsize(REMOTE_RESP)), None
    if (resp:=struct.unpack(REMOTE_RESP, msg))[0] != 0:
  ...
  def _bulk_write(self, cmd:int, idx:int, offset:int, data:bytes):
    ...
    self.sock.sendall(struct.pack(REMOTE_REQ, cmd, self.dev_id, idx, offset, len(data), 0) + data)
...
class APLRemotePCIDevice(RemotePCIDevice):
  """
  The TinyGPU.app path on macOS: a signed server process holds the dext's user client, maps the BARs and creates the dma memory.
  MMIO goes over the socket, dma memory comes back as shared memory (an fd) and is mmapped here, so it is plain memory for the host.
  """
  ...
  def __init__(self, devpref:str, pcibus:str):
    # a custom socket path means a custom server: nothing to install or spawn
    if not (custom:=getenv("APL_REMOTE_SOCK", "")): self.ensure_app()
    sock_path, sock = custom or temp("tinygpu.sock"), socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
    for i in range(100):
      with contextlib.suppress(ConnectionRefusedError, FileNotFoundError):
        sock.connect(sock_path)
        break
      if i == 0 and not custom: subprocess.Popen([self.APP_PATH, "server", sock_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
      time.sleep(0.05)
    else: raise RuntimeError(f"Failed to connect to TinyGPU server at {sock_path}.")
    RemotePCIDevice._stats()
    super().__init__(devpref, "usb4", sock=sock)

    # the server keeps at most 128 dma allocations per session and never frees one, so dma memory is carved out of big chunks here
    self.chunk_size = getenv("APL_SYSMEM_CHUNK", 64) << 20
    # (cpu base, size, 4KB page addresses as the device sees them, allocator or None for a mapping that is one allocation)
    self.chunks:list[tuple[int, int, list[int], TLSFAllocator|None]] = []

  def _map_chunk(self, size:int, managed:bool=True) -> tuple[int, list[int]]:
    mapped_size, _, _, fd = self._rpc(self.sock, self.dev_id, RemoteCmd.MAP_SYSMEM_FD, size, 0, has_fd=True)
    base = FileIOInterface(fd=unwrap(fd)).mmap(0, mapped_size, mmap.PROT_READ | mmap.PROT_WRITE, mmap.MAP_SHARED, 0)

    # the server leaves (address, size) pairs and a (0, 0) terminator at the start of the mapping
    words = MMIOInterface(base, mapped_size, fmt='Q')
    pairs = list(itertools.takewhile(lambda p: p[1] != 0, zip(words[0:2048:2], words[1:2048:2])))
    pages = [p + i for p, sz in pairs for i in range(0, sz, 0x1000)]
    if len(pages) < mapped_size // 0x1000: raise RuntimeError(f"remote: dma mapping covers {len(pages)} of {mapped_size // 0x1000} pages")
    self.chunks.append((base, mapped_size, pages:=pages[:mapped_size // 0x1000], TLSFAllocator(mapped_size, base=base) if managed else None))
    if DEBUG >= 2: print(f"remote: dma chunk {len(self.chunks) - 1}: {mapped_size >> 20} MB at {base:#x}, {len(pairs)} segments")
    return base, pages

  def _find_chunk(self, addr:int, size:int):
    return next(((base, csize, pages, tlsf) for base, csize, pages, tlsf in self.chunks if base <= addr and addr + size <= base + csize), None)

  def alloc_sysmem(self, size:int, vaddr:int=0, contiguous:bool=False) -> tuple[MMIOInterface, list[int]]:
    size = round_up(size, 0x1000)
    if size > self.chunk_size // 2: # a big one gets a mapping of its own: the allocator's alignment slack would not fit it in a chunk
      base, pages = self._map_chunk(size, managed=False)
      return MMIOInterface(base, size, fmt='B'), pages[:size // 0x1000]
    for _ in range(2):
      for base, _, pages, tlsf in self.chunks:
        if tlsf is None: continue
        try: addr = tlsf.alloc(size, 0x1000)
        except MemoryError: continue
        return MMIOInterface(addr, size, fmt='B'), pages[(addr - base) // 0x1000:(addr - base + size) // 0x1000]
      self._map_chunk(self.chunk_size)
    raise MemoryError(f"remote: cannot allocate {size:#x} bytes of dma memory")

  def sysmem_paddrs(self, addr:int, size:int) -> list[int]:
    if (chunk:=self._find_chunk(addr, size)) is None: raise RuntimeError(f"{addr:#x} is not dma memory of {self.pcibus}")
    base, _, pages, _ = chunk
    return pages[(addr - base) // 0x1000:(addr - base + size) // 0x1000]

  def free_sysmem(self, view:MMIOInterface):
    if isinstance(view, RemoteMMIOInterface): return # a bar window, nothing was allocated
    if (chunk:=self._find_chunk(view.addr, 0)) is not None and chunk[3] is not None: chunk[3].free(view.addr)

  bar_from_cfg:bool = bool(getenv("APL_BAR_CFG", 0)) # the server reports its own mapping address: this reads the bus address from config space

  @functools.cache
  def bar_info(self, bar_idx:int) -> tuple[int, int]:
    addr, size = self._rpc(self.sock, self.dev_id, RemoteCmd.MAP_BAR, bar=bar_idx)[:2]
    if self.bar_from_cfg:
      lo = self.read_config(pci.PCI_BASE_ADDRESS_0 + 4 * bar_idx, 4)
      addr = (((self.read_config(pci.PCI_BASE_ADDRESS_0 + 4 * bar_idx + 4, 4) << 32) if lo & pci.PCI_BASE_ADDRESS_MEM_TYPE_64 else 0) | lo) & ~0xf
    return addr, size
